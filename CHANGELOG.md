...
- [`beb7ee2`](https://github.com/clamp-sh/clamp/commit/beb7ee26987c95a1f82c76333867a08ea6069be9) Thanks [@sbj-o](https://github.com/sbj-o)! - Six new skills covering causal reasoning, Bayesian experimentation, sequential monitoring, and time-series anomaly detection. All vendor-neutral; work without Clamp connected, but auto-load via methodology hints when Clamp MCP returns interpretation-needing data.

  - `causal-query-classifier` — Pearl's three-rung hierarchy. Tags every question rung-1 (association), rung-2 (intervention), or rung-3 (counterfactual). Refuses to escalate a rung-1 finding into a rung-2 recommendation without naming an identification strategy (holdback, DiD, IV, synthetic control).
  ...
  - `causal-evidence-checklist` — Bradford Hill's 9 viewpoints (strength, consistency, specificity, temporality, dose-response, plausibility, coherence, experiment, analogy) as a rubric before recommending a rollback/ship action. Refuses "high confidence" verdicts when fewer than ~5 criteria pass.
  - `bayesian-experiment-reader` — Beta-Binomial / Normal-Normal posteriors, P(variant beats control), expected-loss decision rule. Ship-if-better-AND-loss-under-tolerance instead of "stat sig yes/no."
  - `sequential-monitoring` — mSPRT and confidence sequences for safe peeking. Answers "can I call this early?" without α-inflation. Pairs with `experiment-result-reader` and `bayesian-experiment-reader`.
  - `anomaly-detection-time-series` — STL decomposition, Bayesian online changepoint detection, Prophet, quantile regression, SPRT, Granger causality. Use when two fingerprints match with similar confidence, the change date is contested, or the user asks "is this real?"

  Existing skill cross-links updated:

  - `analytics-diagnostic-method` `when_to_use` references the new causal + Bayesian skills as pair-load candidates.
  - `traffic-change-diagnosis` references `anomaly-detection-time-series` for ambiguous fingerprints.
  - `experiment-result-reader` references `bayesian-experiment-reader` + `sequential-monitoring`.
  - `channel-and-funnel-quality` references `causal-dag-builder` + `causal-evidence-checklist` before declaring causality.
