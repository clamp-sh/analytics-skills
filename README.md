...
The skills are analytics-agnostic but include Clamp-specific tool call-outs. If you have [Clamp MCP](https://clamp.sh/mcp) connected, the agent knows exactly which tool to reach for at each step of the method (`traffic.overview`, `referrers.top`, `funnels.create`, etc.).
