...
| Channel split | `referrers.top`, `traffic.breakdown` |
| Cohort split | `traffic.breakdown` (by new vs returning, geo, device) |
| Time shape | `traffic.timeseries` (hourly or daily granularity) |
| Funnel check | `funnels.create` / `funnels.get` |
| Before / after comparison | `mcp_clamp_traffic_compare` |
