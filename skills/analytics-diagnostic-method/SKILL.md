...
| Step | Clamp tool |
|---|---|
| Measurement sanity check | `get_events` (is the conversion event firing?), `get_overview` (do totals make sense?) |
| Channel split | `get_top_referrers`, `get_breakdown` |
| Cohort split | `get_breakdown` (by new vs returning, geo, device) |
| Time shape | `get_timeseries` (hourly or daily granularity) |
| Funnel check | `create_funnel` / `get_funnel` |
| Before / after comparison | `mcp_clamp_compare_periods` |
| Live check during an incident | `get_current_visitors` |
