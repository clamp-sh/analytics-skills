...
(Affects metric definitions, especially around bounce/engagement and session stitching. Also picks the `tool_map` for downstream skills.)
...
If the codebase has more than a handful of `track()`-style calls and no `event-schema.yaml`, also suggest `event-schema-author`. Declaring events in one place is foundational for everything else (typo-free names, refactor-safe properties, intent that survives the original author).
...
- **`tool_map`**: <one of: clamp | posthog | mixpanel | amplitude | ga4>
...
Agent: [scans for available MCP namespaces; sees `mcp__clamp__*` is connected]
I see Clamp MCP is already connected; planning to record `tool_map: clamp`.
Anything else in the stack we should note (Stripe, server logs, CRM)?

User: yep, Clamp plus Stripe for revenue
...
## Verifying answers via the active MCP

Whichever analytics MCP is connected (Clamp, PostHog, Mixpanel, Amplitude, GA4), Phase 3 can verify the user's answers directly using the row-2 / row-4 / row-1 invocations from the matching `tool-maps/<name>.md`:
