...
The method above is platform-neutral. For specific MCP calls per workflow row (channel split, funnel construction, cohort retention, attribution, path analysis), load the tool-map for the active platform: see [`tool-maps/`](../../tool-maps/) and the `tool_map:` field in `analytics-profile.md`. If the field is missing, run `analytics-profile-setup` to set it.
