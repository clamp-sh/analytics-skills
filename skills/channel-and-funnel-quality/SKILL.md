...
| Check | Clamp tool |
|---|---|
| Channel list and volumes | `referrers.top` |
| Per-channel engagement | `pages.engagement` + `traffic.breakdown` by referrer |
| Country/device splits | `countries.top`, `cities.top`, `devices.top` |
| New vs returning split | `traffic.breakdown` |
| Build / inspect a funnel | `funnels.create`, `funnels.get` |
| Campaign period comparison | `mcp_clamp_traffic_compare` |
| Revenue by channel (if Stripe wired) | `revenue.sum` + `traffic.breakdown` |
