...
### 3. When section-view tracking is available, triangulate

If the analytics tool tracks section views (e.g. Clamp's section-views extension fires `section_viewed` per marked section), use them as a secondary engagement signal — but only investigate low section views when downstream conversion is *also* weak. Below-fold redundancy is not a leak: a section that few visitors see, but whose upstream CTAs convert normally, is doing its job for the audience that needs it.

### 4. Is the step actually being measured correctly?
...
| Check | Clamp tool |
|---|---|
| Channel list and volumes | `breakdown(dimension="referrer_host")` (channel comes back joined per row) or `breakdown(dimension="channel")` |
| Per-channel engagement | `pages.engagement(view="engagement")` filtered by `channel` or `referrer_host` |
| Country / device splits | `breakdown(dimension="country" \| "city" \| "device_type" \| "browser" \| "os")` |
| New vs returning split | `breakdown(dimension=...)` with the cohort filter |
| Build / inspect a funnel | `funnels.create`, `funnels.get` |
| Campaign period comparison | `traffic.compare` |
| Revenue by channel (if Stripe wired) | `revenue.sum(group_by="channel" \| "referrer_host")` |
| Path analysis: what do they actually do? | `sessions.paths` |
| Section-level attention on a landing page | `pages.engagement(view="sections", pathname=...)` — pair with CTA-click events to spot wasted real estate |
