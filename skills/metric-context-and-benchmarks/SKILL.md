...
| Question | Clamp tool |
|---|---|
| Aggregate metrics (sessions, pageviews, bounce/engagement) | `get_overview` |
| Per-page engagement (where is bounce concentrated?) | `get_page_engagement`, `get_top_pages` |
| Conversion / event volume | `get_events` |
| Revenue + ARPU / RPV | `get_revenue` |
| By-segment rates (device, geo, referrer, new vs returning) | `get_breakdown` |
| Trailing trend for noise-range check | `get_timeseries` |
