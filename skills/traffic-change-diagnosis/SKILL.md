...
| Check | Clamp tool |
|---|---|
| Realtime sanity | `get_current_visitors` |
| Time shape (hourly or daily) | `get_timeseries` with appropriate interval |
| Before/after comparison of the drop window | `mcp_clamp_compare_periods` |
| Channel localization | `get_top_referrers` |
| Page / landing page localization | `get_top_pages` |
| Geo / device / new-vs-returning cohorts | `get_breakdown` |
| Country-level bot spike check | `get_countries`, `get_cities` |
| Which events still fire | `get_events` |
| Engagement-level bot filter | `get_page_engagement` (look for sessions with ~0s duration) |
