...
| Before/after comparison of the drop window | `traffic.compare` |
| Channel localization | `breakdown(dimension="referrer_host")` (channel comes back joined per row) |
| Page / landing page localization | `pages.engagement` (default `view="summary"`) |
| Geo / device / new-vs-returning cohorts | `breakdown(dimension="country" \| "device_type" \| ...)` |
| Country-level bot spike check | `breakdown(dimension="country")`, `breakdown(dimension="city")` |
| Which events still fire | `events.list` |
| Engagement-level bot filter | `pages.engagement(view="engagement")` (look for sessions with ~0s duration) |
